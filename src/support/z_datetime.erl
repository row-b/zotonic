...
relative_time(N, '+', ["minute" | _], Now) ->
    relative_time_n(N, fun next_minute/1, Now);
relative_time(N, '+', ["hour" | _], Now) ->
    relative_time_n(N, fun next_hour/1, Now);
relative_time(N, '+', ["day" | _], Now) ->
    relative_time_n(N, fun next_day/1, Now);
relative_time(N, '+', ["sunday" | _], Now) ->
    relative_time_n(N * 7, fun next_day/1, week_start(7, Now));
relative_time(N, '+', ["monday" | _], Now) ->
    relative_time_n(N * 7, fun next_day/1, week_start(1, Now));
relative_time(N, '+', ["tuesday" | _], Now) ->
    relative_time_n(N * 7, fun next_day/1, week_start(2, Now));
relative_time(N, '+', ["wednesday" | _], Now) ->
    relative_time_n(N * 7, fun next_day/1, week_start(3, Now));
relative_time(N, '+', ["thursday" | _], Now) ->
    relative_time_n(N * 7, fun next_day/1, week_start(4, Now));
relative_time(N, '+', ["friday" | _], Now) ->
    relative_time_n(N * 7, fun next_day/1, week_start(5, Now));
relative_time(N, '+', ["saturday" | _], Now) ->
    relative_time_n(N * 7, fun next_day/1, week_start(6, Now));
relative_time(N, '+', ["week" | _], Now) ->
    relative_time_n(N * 7, fun next_day/1, Now);
relative_time(N, '+', ["month" | _], Now) ->
    relative_time_n(N, fun next_month/1, Now);
relative_time(N, '+', ["year" | _], Now) ->
    relative_time_n(N, fun next_year/1, Now);
relative_time(N, '-', ["day" | _], Now) ->
    relative_time_n(N, fun prev_day/1, Now);
relative_time(N, '-', ["sunday" | _], Now) ->
    relative_time_n(N * 7, fun prev_day/1, week_start(7, Now));
relative_time(N, '-', ["monday" | _], Now) ->
    relative_time_n(N * 7, fun prev_day/1, week_start(1, Now));
relative_time(N, '-', ["tuesday" | _], Now) ->
    relative_time_n(N * 7, fun prev_day/1, week_start(2, Now));
relative_time(N, '-', ["wednesday" | _], Now) ->
    relative_time_n(N * 7, fun prev_day/1, week_start(3, Now));
relative_time(N, '-', ["thursday" | _], Now) ->
    relative_time_n(N * 7, fun prev_day/1, week_start(4, Now));
relative_time(N, '-', ["friday" | _], Now) ->
    relative_time_n(N * 7, fun prev_day/1, week_start(5, Now));
relative_time(N, '-', ["week" | _], Now) ->
    relative_time_n(N * 7, fun prev_day/1, Now);
relative_time(N, '-', ["month" | _], Now) ->
    relative_time_n(N, fun prev_month/1, Now);
relative_time(N, '-', ["year" | _], Now) ->
    relative_time_n(N, fun prev_year/1, Now);
    ...
    case diff(D1, D2) of
        ...
        {{0, M, D}, {_, _, _}} ->
            {plural(M, ?__(<<"month">>, Context), ?__(<<"months">>, Context)),
                plural(D, ?__(<<"day">>, Context), ?__(<<"days">>, Context))};
        {{Y, M, _}, {_, _, _}} ->
            {plural(Y, ?__(<<"year">>, Context), ?__(<<"years">>, Context)),
                plural(M, ?__(<<"month">>, Context), ?__(<<"months">>, Context))}
    end.
...
week_start(StartDayNr, {D, _}) ->
    ...
    if
        WeekDay > StartDayNr ->
            relative_time_n(WeekDay - StartDayNr, fun prev_day/1, Today);
        WeekDay =:= StartDayNr -> Today;
        WeekDay < StartDayNr ->
            relative_time_n(WeekDay - StartDayNr + 7, fun prev_day/1, Today)
    end.
