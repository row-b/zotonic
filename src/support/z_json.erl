...
to_mochijson([{_, _} | _] = PS, Context) ->
    {struct, [{K, to_mochijson(V, Context)} || {K, V} <- PS]};
to_mochijson(L, Context) when is_list(L) ->
    {array, [to_mochijson(V, Context) || V <- L]};
to_mochijson({trans, _} = Tr, Context) -> z_trans:lookup_fallback(Tr, Context);
to_mochijson({{_, _, _}, {_, _, _}} = Date, Context) ->
...
%% @todo Translate
from_mochijson({struct, Props}, Context) ->
    [opt_convert({K, from_mochijson(V, Context)}) || {K, V} <- Props];
from_mochijson({array, Values}, Context) ->
    [from_mochijson(V, Context) || V <- Values];
