...
%%                     ignore               |
%%                     {stop, Reason}
%% @doc Initiates the server.  Options are: dropbox_dir, processing_dir,
%% unhandled_dir, interval, max_age and min_age
init(SiteProps) ->
    ...
    DropBox = string:strip(
        proplists:get_value(dropbox_dir, SiteProps, DefaultDropBoxDir),
        right,
        $/
    ),
    ProcDir = string:strip(
        proplists:get_value(dropbox_processing_dir, SiteProps, DefaultProcessingDir),
        right,
        $/
    ),
    UnDir = string:strip(
        proplists:get_value(dropbox_unhandled_dir, SiteProps, DefaultUnhandledDir),
        right,
        $/
    ),
    State = #state{
...
do_scan(#state{
    ...
    {ToProcess, ToRemove} = lists:foldl(fun(F, Acc) ->
        max_age_split(F, MaxAge, Acc) end,
        {[], []},
        ProcFiles),
    lists:foreach(fun(F) ->
        move_file(ProcDir, F, true, UnhandledDir) end, ToRemove),

    % Move all new dropbox files to the processing directory
    AllDropFiles = scan_directory(DropDir),
    SafeDropFiles = lists:foldl(fun(F, Acc) ->
        min_age_check(F, MinAge, Acc) end,
        [],
        AllDropFiles),
    Moved = lists:map(fun(F) ->
        {F, move_file(DropDir, F, false, ProcDir)} end, SafeDropFiles),
    ToProcess1 = lists:foldl(fun
        ...
        Moved),
    lists:foreach(fun(F) ->
        z_notifier:first(#dropbox_file{filename = F}, State#state.context) end, ToProcess1).


%% @doc Scan a directory, return list of files not changed in the last 10 seconds.
scan_directory(Dir) ->
    filelib:fold_files(Dir, "", true, fun(F, Acc) ->
        append_file(F, Acc) end, []).


%% @todo Check if this is a file we are interested in, should not be part of a
%% .svn or other directory
