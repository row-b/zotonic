...
scan_paths(Paths) ->
    Files = lists:foldl(fun(L, Acc) ->
        L ++ Acc end, [], [z_utils:wildcard(filename:join(P)) || P <- Paths]),
    ...
    case lists:filter(fun(M) -> is_startable(M, Provided) end, Starting) of
        [] ->
            ...
            handle_start_next(State#state{
                start_error = [{M, case is_module(M) of false ->
                    not_found; true -> dependencies_error end}
                    || M <- Starting
                ] ++ CleanedUpErrors,
                start_queue = []
            });
