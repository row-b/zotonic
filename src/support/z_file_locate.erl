...
locate_sources(Root, ImageFilters, Files, Context) ->
    ...
    [locate_source(Root, Path, OriginalFile, Filters, Context) ||
        {Path, OriginalFile, Filters} <- FilesFilters].
                ...
                {_, _} ->
                    case z_media_tag:url2props(SafePath, Context) of
                        {ok, {OriginalFile, PreviewPropList, _Checksum, _ChecksumBaseString}} ->
                            {SafePath, OriginalFile, case OptFilters of undefined ->
                                []; _ -> OptFilters end ++ PreviewPropList};
                        {error, _} ->
                            {SafePath, SafePath, OptFilters}
                    end
            end
...
locate_source(NoRoots, Path, "lib/" ++ OriginalFile, Filters, Context)
    when NoRoots =:= undefined; NoRoots =:= [] ->
    locate_source([lib], Path, OriginalFile, Filters, Context);
locate_source(NoRoots, Path, <<"lib/", OriginalFile/binary>>, Filters, Context)
    when NoRoots =:= undefined; NoRoots =:= [] ->
    locate_source([lib], Path, OriginalFile, Filters, Context);
locate_source(NoRoots, Path, OriginalFile, Filters, Context)
    when NoRoots =:= undefined; NoRoots =:= [] ->
...
locate_source([ModuleIndex | Roots], Path, OriginalFile, Filters, Context)
    when is_atom(ModuleIndex) ->
...
%% @doc Source file is located in the lib, template or some other
%% index-category (mostly css, js or static images)
...
maybe_generate_preview(Path, OriginalFile, Filters, Medium, Context) ->
    ...
    generate_preview(
        z_media_preview:can_generate_preview(Mime),
        Mime,
        Path,
        OriginalFile,
        Filters,
