...
%% @doc Make still previews of media, using image manipulation functions.
%% Resize, crop, grey, etc.
%% This uses the command line imagemagick tools for all image manipulation.
%% This code is adapted from PHP GD2 code, so the resize/crop could've been done
%% more efficiently, but it works :-)
...
can_generate_preview(B) when is_binary(B) ->
    can_generate_preview(binary_to_list(B));
...
move_pre_post_filters(Fs) ->
    ...
    ++ lists:filter(fun(F) ->
        not is_pre_filter(F) andalso not is_post_filter(F) end, Fs)
...
get_lossless_value(Options) ->
    ...
    case ValueOpt of
        undefined ->
            z_convert:to_atom(proplists:get_value(lossless, Options, false));
        V -> V
    end.
...
filter2arg({blur, Blur}, Width, Height, _AllFilters) when is_list(Blur) ->
    case string:tokens(Blur, "x") of
        [A, B] ->
            {Width, Height, ["-blur ", ensure_integer(A), $x, ensure_integer(B)]};
...
fetch_crop(Filters) ->
    ...
    CropPar = case Crop of
        [{crop, None} | _] when None == false; None == undefined; None == ""; None == <<>> ->
            none;
    ...
    case CropPar of
        ...
        _ ->
            ...
            CropL = case CropPar of
                ...
                X when X == north_east; X == east; X == south_east ->
                    ceil(W - Width);
                [X, _] when is_integer(X) ->
                    ceil(erlang:max(0, erlang:min(W - Width, X / Scale - Width / 2)));
                _ -> ceil((W - Width) / 2)
            end,

            CropT = case CropPar of
                Y when Y == north_west; Y == north; Y == north_east -> 0;
                Y when Y == south_west; Y == south; Y == south_east ->
                    ceil(H - Height);
                [_, Y] when is_integer(Y) ->
                    ceil(erlang:max(0, erlang:min(H - Height, Y / Scale - Height / 2)));
                _ -> ceil((H - Height) / 2)
            end,
