...
handle_waiting_children(#state{waiting = Waiting} = State) ->
    lists:foldl(fun(C, S) ->
        do_start_child(C, S) end, State#state{waiting = []}, Waiting).
...
handle_retrying_children(#state{retrying = Retrying} = State) ->
    Now = z_utils:now(),
    {Start, Wait} = lists:partition(fun(CS) ->
        is_ready_for_retry(CS, Now) end, Retrying),
...
handle_failed_children(#state{failed = Failed} = State) ->
    Now = z_utils:now(),
    {Start, Fail} = lists:partition(fun(CS) ->
        is_ready_for_unfail(CS, Now) end, Failed),
    lists:foldl(fun(#child_state{child = Child} = CS, S) ->
        case start_child_mfa(Child#child_spec.mfa) of
            {ok, Pid} ->
