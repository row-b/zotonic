...
purge_monitor_path(Ref, {file, Path}, St) ->
    St#state{files = purge_monitor_path1(Path, Ref, St#state.files)};
purge_monitor_path(Ref, {directory, Path}, St) ->
    St#state{dirs = purge_monitor_path1(Path, Ref, St#state.dirs)}.

purge_monitor_path1(Path, Ref, Dict) ->
...
event(OldEntry, #entry{info = NewInfo} = Entry, Type, Path, St) ->
    ...
    case changed(Diff, OldEntry, Entry, NewInfo) of
        true ->
...
%% @doc if only the time-to-stable has changed, we must not %% broadcast an
%% event unless there really was a change in the directory list
changed(Diff, _OldEntry, _NewEntry, _NewInfo) when Diff =/= [] ->
    true;
changed(_Diff, #entry{stable = Stable}, #entry{stable = Stable}, _NewInfo) ->
    true;
changed(_Diff, #entry{info = Info}, _NewEntry, NewInfo) when Info =/= NewInfo ->
    true;
changed(_Diff, _OldEntry, _Entry, _NewInfo) ->
    false.
...
refresh_entry(Path, Entry, Type, Delta) when is_binary(Path) ->
    refresh_entry0(binary_to_list(Path), Entry, Type, Delta,
        Entry#entry.info).

refresh_entry0(Path, Entry, Type, Delta, OldInfo) ->
    refresh_entry1(Path, Entry, Type, Delta, OldInfo, false).

refresh_entry1(Path, Entry, Type, Delta, OldInfo, Break) ->
        ...
        directory when not is_atom(NewInfo) ->
            case NewInfo#file_info.type of
                directory when is_atom(OldInfo)
                    ...
                    refresh_entry2(Path, Entry, Type, Delta, NewInfo,
                        ?TIME_TO_STABLE);
...
refresh_entry2(Path, Entry, Type, Delta, Info, Stable) ->
...
list_dir(Path) when is_list(Path) ->
    Files = case file:list_dir(Path) of
        {ok, Fs} ->
            [normalize_path(F) || F <- Fs, F =/= <<"files">>, F =/= "files"];
        {error, _} -> []
    end,
    lists:sort(Files).
