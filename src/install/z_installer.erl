...
install_acl(C, Database, Schema) ->
    ...
    case HasRscGroup andalso HasGroup of
        true ->
            ...
            {ok, 1} = epgsql:equery(
                C,
                "insert into module (name, is_active) values ($1, true)", ["mod_acl_adminonly"]
            ),
            ok;
        false ->
            ok
...
extent_mime(C, Database, Schema) ->
    ...
    case Length < 128 of
        true ->
            {ok, [], []} = epgsql:squery(
                C,
                "alter table medium alter column mime type character varying(128)"
            );
        false ->
            nop
    end,
...
install_identity_is_verified(C, Database, Schema) ->
    case has_column(C, "identity", "is_verified", Database, Schema) of
        ...
        false ->
            ...
            {ok, [], []} = epgsql:squery(
                C,
                "update identity set is_verified = true where key = 'username_pw'"
            ),
            ok
    end.
...
install_task_due(C, Database, Schema) ->
    case has_column(C, "pivot_task_queue", "due", Database, Schema) of
        ...
        false ->
            {ok, [], []} = epgsql:squery(
                C,
                "alter table pivot_task_queue add column due timestamp with time zone"
            ),
            ok
    end.
...
install_module_schema_version(C, Database, Schema) ->
    case has_column(C, "module", "schema_version", Database, Schema) of
        ...
        false ->
            {ok, [], []} = epgsql:squery(
                C,
                "alter table module add column schema_version int "
            ),
            Predefined = [
                "mod_twitter",
                "mod_mailinglist",
            ...
            [
                {ok, _} = epgsql:equery(
                    C,
                    "UPDATE module SET schema_version=1 WHERE name=$1 AND is_active=true", [M]
                ) || M <- Predefined
            ],
            ok
    end.
...
fix_timestamptz_column(C, Table, Col, Database, Schema) ->
    ...
    {ok, [], []} = epgsql:squery(
        C,
        "alter table " ++ binary_to_list(Table) ++ " alter column " ++
            binary_to_list(Col) ++ " type timestamp with time zone"
    ),
    ok.
...
install_content_group_dependent(C, Database, Schema) ->
    case has_column(C, "rsc", "content_group_id", Database, Schema) of
        ...
        false ->
            lager:info(
                "[database: ~p ~p] Adding rsc.is_dependent and rsc.content_group_id",
                [Database, Schema]
            ),
            {ok, [], []} = epgsql:squery(C,
                "ALTER TABLE rsc "
...
convert_category_hierarchy(C, Database, Schema) ->
    case has_table(C, "hierarchy", Database, Schema) of
        false ->
            ...
            {ok, [], []} = epgsql:squery(C, z_install:hierarchy_index1()),
            {ok, [], []} = epgsql:squery(C, z_install:hierarchy_index2()),
