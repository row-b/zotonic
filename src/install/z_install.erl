    ...
    [
       ...
       (
        ...
        (
          ...
          publication_end timestamp with time zone NOT NULL
                DEFAULT '9999-06-01 00:00:00'::timestamp with time zone,
       ...
       (
                ...
                loop
                    ...
                    begin
                        insert into rsc_pivot_queue (rsc_id, due, is_update)
                            values (new.id, now(), tg_op = 'UPDATE');
        ...
        % Queue for deleted medium files, periodically checked for deleting
        % files that are not referenced anymore
        ...
        begin
            if (tg_op = 'DELETE') then
                ...
                if (old.preview_filename <> '' and old.preview_filename is not
                    null and old.is_deletable_preview) then
                    insert into medium_deleted (filename)
                    values (old.preview_filename);
                end if;
            end if;
            return null;
        end;
        ...
        % Table with all uploaded filenames, used to ensure unique filenames in
        % the upload archive
        medium_log_table(),
...
edge_log_function() ->
    ...
    begin
        if (tg_op = 'INSERT') then
            ...
            values (tg_op, new.id, new.subject_id, new.object_id, new.predicate_id, new_predicate,
                new.seq);
        ...
        elseif (tg_op = 'DELETE') then
            ...
            insert into edge_log (op, edge_id, subject_id, object_id, predicate_id,
                predicate, seq, created)
            values (tg_op, old.id, old.subject_id, old.object_id, old.predicate_id,
                new_predicate, old.seq, old.created);
        end if;
        return null;
    end;
    $$ LANGUAGE plpgsql
...
medium_update_function() ->
    ...
    begin
        ...
        if (tg_op = 'INSERT') then
            ...
            if (new.preview_filename <> '' and new.preview_filename is not null
            and new.is_deletable_preview) then
                insert into medium_log (filename, usr_id)
                values (new.preview_filename, user_id);
            end if;
        elseif (tg_op = 'UPDATE') then
            if (new.filename <> '' and new.filename is not null and new.is_deletable_file "
                ++ "and new.filename != old.filename) then
                insert into medium_log (filename, usr_id)
                values (new.filename, user_id);
            end if;
            if (new.preview_filename <> '' and new.preview_filename is not null
            and new.is_deletable_preview and new.preview_filename != old.preview_filename) then
                insert into medium_log (filename, usr_id)
                values (new.preview_filename, user_id);
            end if;
            -- Insert files into the medium_deleted queue table
            if (old.filename <> '' and old.filename is not null and old.is_deletable_file "
                ++ "and new.filename != old.filename) then
                insert into medium_deleted (filename) values (old.filename);
            end if;
            if (old.preview_filename <> '' and old.preview_filename is not null
            and old.is_deletable_preview and new.preview_filename != old.preview_filename) then
