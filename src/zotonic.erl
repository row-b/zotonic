...
ensure_mnesia_schema() ->
    ...
    case application:get_env(mnesia, dir) of
        undefined ->
            lager:info(
                "No mnesia directory defined, running without persistent email queue and "
                "filezcache.~nTo enable persistency, add to erlang.config: "
                "{mnesia,[{dir,\"priv/mnesia\"}]}~n~n"
            ),
            ok;
        {ok, Dir} ->
            case filelib:is_dir(Dir)
                andalso filelib:is_regular(filename:join(Dir, "schema.DAT")) of
                true ->
                    ok;
                false ->
