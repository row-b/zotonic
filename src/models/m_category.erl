    ...
    F = fun
        ...
        (Nm, Acc) ->
            case get(Nm, Context) of
                undefined -> [{-1, -1} | Acc];
                Props ->
                    [{proplists:get_value(left, Props), proplists:get_value(right, Props)} | Acc]
            end
    end,
...
%% @doc Check if a category is a meta category. This can't use the m_rsc
%% routines as it is also
