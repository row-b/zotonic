...
t_l1([[{{ws, comp}, W},
    ...
    case gt(W, 3) of
        ...
        false ->
            t_l1(T1, A1, [{normal, H} | A2]) % same exit at the final clause!
    end,
    t_inline(H, T1, T2, A1, A2);
...
t_inline(H, T1, T2, A1, A2) ->
    case snip_ref(T1) of
        {Type, {Id, {Url, Title}}} ->
            t_l1(T2, flatten([{Id, {Url, Title}} | A1]),
                [{Type, H} | A2]);
...
esc_t1([?NBSP | T], Acc) ->
    esc_t1(T, [?SPACE | Acc]); % non-breaking space to space
...
get_url(String) -> HTTPRegex = "^(H|h)(T|t)(T|t)(P|p)(S|s)*://",
    case re:run(String, HTTPRegex) of
        nomatch -> not_url;
...
get_email_addie(String) ->
    SnipRegex = ">",
    case re:run(String, SnipRegex) of
        nomatch -> not_email;
        {match, [{N, _} | _T]} ->
            ...
            case re:run(Possible, EmailRegex) of
                nomatch -> not_email;
