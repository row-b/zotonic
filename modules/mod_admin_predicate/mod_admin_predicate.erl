    ...
    case z_acl:rsc_deletable(PredId, Context) of
        true ->
            ...
            case z_acl:is_allowed(
                insert,
                #acl_edge{subject_id = PredId, predicate = ToPredName, object_id = PredId},
                Context)
            of
                true ->
                ...
                false ->
                    z_render:growl(
                        ?__("Sorry, you are not allowed to insert connections "
                            "with this predicate.", Context),
                        Context
                    )
            end;
        false ->
            z_render:growl(
                ?__("Sorry, you are not allowed to delete this.", Context),
                Context
            )
    end;
    ...
    case not IfEmpty orelse not m_predicate:is_used(PredId, Context) of
        true ->
            case z_acl:rsc_deletable(PredId, Context) of
                true ->
                    Context1 = z_context:prune_for_async(Context),
                    spawn(fun() -> pred_delete(PredId, Context1) end),
                    z_render:wire({dialog_close, []}, Context);
                false ->
                    z_render:growl(
                        ?__("Sorry, you are not allowed to delete this.", Context),
...
%% @doc Check if the update contains information for a predicate. If so then
%% update the predicate information in the db and remove it from the update
%% props.
