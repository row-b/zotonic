...
observe_search_query(#search_query{search = {mailinglist_bounced, [{list_id, ListId}, {mailing_id, Id}]}}, _Context) ->
    #search_sql{
...
observe_email_bounced(B = #email_bounced{}, Context) ->
    ...
    lists:foreach(fun(Id) ->
        m_mailinglist:update_recipient(Id, [{is_enabled, false}, {is_bounced, true}, {bounce_time, calendar:universal_time()}], Context) end, Recipients),
...
send_mailing(ListId, PageId, Context) ->
    spawn(fun() ->
        send_mailing_process(ListId, PageId, z_acl:sudo(Context)) end).
