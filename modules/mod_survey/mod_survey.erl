...
event(#submit{message = {survey_next, Args}}, Context) ->
    ...
    Editing = proplists:get_value(editing, Args),
    render_update(
        render_next_page(SurveyId, PageNr + 1, forward, Answers, History, Editing, Context),
        Args,
        Context
    );
...
event(#postback{message = {survey_back, Args}}, Context) ->
    ...
    case History of
        [_, PageNr | History1] ->
            render_update(
                render_next_page(SurveyId, PageNr, exact, Answers, History1, Editing, Context),
                Args,
                Context
            );
        _History ->
            render_update(
                render_next_page(SurveyId, 0, exact, Answers, [], Editing, Context),
                Args,
                Context
            )
    end;
...
event(#postback{
        message = {survey_remove_result, [{id, SurveyId}, {persistent_id, PersistentId}, {user_id, UserId}]}
    },
    Context) ->
...
observe_survey_is_submit(#survey_is_submit{block = Q}, _Context) ->
    case proplists:get_value(type, Q) of
        <<"survey_button">> -> true;
        <<"survey_", _/binary>> ->
            proplists:get_value(input_type, Q) =:= <<"submit">>;
        _ -> undefined
    end.


%% @doc Fetch the filename for the export
observe_export_resource_filename(
    #export_resource_filename{dispatch = survey_results_download, id = Id},
    Context) ->
...
observe_export_resource_header(
    #export_resource_header{dispatch = survey_results_download, id = Id},
    Context) ->
    case m_survey:is_allowed_results_download(Id, Context) of
        true ->
...
observe_export_resource_data(
    #export_resource_data{dispatch = survey_results_download, state = Data},
    _Context) ->
    {ok, Data, undefined};
...
normalize_answer(A) when is_atom(A), is_binary(A) ->
...
-spec render_next_page(integer(), integer(), exact|forward, list(), list(), list()|undefined, #context{}) ->
    #render{} | #context{}.
...
render_next_page(Id, PageNr, Direction, Answers, History, Editing, Context) when is_integer(Id) ->
    ...
    Answers1 = lists:foldl(fun({Arg, _Val}, Acc) ->
        proplists:delete(Arg, Acc) end, Answers, As),
            ...
            case Next of
                ...
                {error, {not_found, Name} = Reason} ->
                    ...
                    z_render:growl_error(
                        "Error in survey, could not find page "
                        ++ z_convert:to_list(Name),
                        Context
                    );
                    ...
                    case do_submit(Id, Questions, Answers2, Context) of
                        ...
                        {error, _Reason} ->
                            #render{
                                ...
                                vars = [{id, Id}, {history, History}, {q, As}]
                            }
                    end;
...
go_button_target(Submitter, Questions, Answers, Context) ->
    [Button | _] = lists:dropwhile(fun(B) ->
        proplists:get_value(name, B) =/= Submitter end, Questions),
