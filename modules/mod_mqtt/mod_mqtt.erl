...
observe_edge_delete(
    #edge_delete{subject_id = SubjectId, predicate = PredName, object_id = ObjectId} = EdgeDelete,
    Context) ->
    z_mqtt:publish(
        <<"~site/rsc/", (z_convert:to_binary(SubjectId))/binary, "/o/",
            (z_convert:to_binary(PredName))/binary>>,
        EdgeDelete,
        Context
    ),
    z_mqtt:publish(
        <<"~site/rsc/", (z_convert:to_binary(ObjectId))/binary, "/s/",
            (z_convert:to_binary(PredName))/binary>>,
        EdgeDelete,
        Context
    ).
...
observe_edge_insert(
    #edge_insert{subject_id = SubjectId, predicate = PredName, object_id = ObjectId} = EdgeInsert,
    Context) ->
    z_mqtt:publish(
        <<"~site/rsc/", (z_convert:to_binary(SubjectId))/binary, "/o/",
            (z_convert:to_binary(PredName))/binary>>,
...
observe_edge_update(
    #edge_update{subject_id = SubjectId, predicate = PredName, object_id = ObjectId} = EdgeUpdate,
    Context
) ->
    z_mqtt:publish(
        <<"~site/rsc/", (z_convert:to_binary(SubjectId))/binary, "/o/",
