...
%% <img class="z-tinymce-media z-tinymce-media-align-block
%% z-tinymce-media-size-small z-tinymce-media-crop- z-tinymce-media-link- "
...
event(
    #postback_notify{message = <<"feedback">>, trigger = <<"dialog-connect-find">>, target = TargetId},
    Context) ->
...
do_link_unlink_error(false = _IsUnlink, Context) ->
    {error, z_render:growl_error(
        ?__("Sorry, you have no permission to add the connection.", Context),
        Context)
    };
do_link_unlink_error(true, Context) ->
    {error, z_render:growl_error(
        ?__("Sorry, you have no permission to delete the connection.", Context),
        Context)
    }.
