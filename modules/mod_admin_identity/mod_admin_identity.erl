...
%% @doc Identity administration.  Adds overview of users to the admin and
%% enables to add passwords on the edit page.
                    ...
                    case m_identity:is_valid_key(Type, KeyNorm, Context) of
                        ...
                        false ->
                            case proplists:get_value(error_target, Args) of
                                ...
                                ErrorTarget ->
                                    z_render:wire(
                                        {add_class, [{target, ErrorTarget}, {class, "has-error"}]},
                                        Context
                                    )
                            end
                    end
...
is_existing_key(RscId, Type, Key, Context) ->
    ...
    case lists:filter(fun(Idn) ->
        proplists:get_value(key, Idn) == Key end, Existing) of
        [] -> false;
        _ -> true
    end.
