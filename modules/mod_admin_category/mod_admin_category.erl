    ...
    case not IfEmpty orelse not m_category:is_used(Id, Context) of
        true ->
            case deletable(Ids, Context) of
                true ->
                    Context1 = z_context:prune_for_async(Context),
                    spawn(fun() -> cat_delete(Ids, Context1) end),
                    z_render:wire({dialog_close, []}, Context);
                false ->
                    z_render:growl(?__("Sorry, you are not allowed to delete this.", Context), Context)
            end;
        false ->
            z_render:wire(
                {alert, [{message, ?__("Delete is canceled, there are pages in the category.", Context)}]},
                Context
            )
    end.
...
in_categories(Ids, Context) when is_list(Ids) ->
    RscIds = z_db:q(
        "select id from rsc where category_id in (SELECT(unnest($1::int[])))",
        [Ids],
        Context,
        60000
    ),
