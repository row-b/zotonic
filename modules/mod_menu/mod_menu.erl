    ...
    case m_rsc:is_visible(FromId, Context) of
        true ->
            ...
            case m_rsc:duplicate(FromId, [{title, NewTitle}], Context) of
                {ok, NewId} ->
                    {Html, Context1} = z_template:render_to_iolist(
                        {cat, "_menu_edit_item.tpl"},
                        [{id, NewId}, {editable, true}],
                        Context
                    ),
                ...
                {error, _Reason} ->
                    z_render:growl_error(
                        ?__("Sorry, can’t copy that page.", Context),
                        Context
                    )
            end;
        false ->
...
handle_cmd(<<"menu-item-render">>, Data, Context) ->
    ...
    {Html, Context2} = z_template:render_to_iolist(
        {cat, "_menu_edit_item.tpl"},
        [{id, Id}, {editable, true}],
        Context
    ),
...
unpack([$, | S], Acc) ->
    unpack(S, Acc);
unpack(S, Acc) ->
    {Id, S1} = lists:splitwith(fun(C) when C == $[; C == $]; C == $, ->
        false; (_) -> true end, S),
    {Rest, Sub} = case S1 of
        [$[ | Xs] -> unpack(Xs, []);
        _ -> {S1, []}
...
remove_invisible([{Id, Sub} | Rest], Acc, Context) ->
    case m_rsc:is_visible(Id, Context) andalso m_rsc:exists(Id, Context) of
        true ->
            remove_invisible(Rest, [{Id, remove_invisible(Sub, [], Context)} | Acc], Context);
...
-spec menu_flat(list() | undefined | <<>>, #context{}) ->
    [{integer()|undefined, LevelIndex :: list(integer()) | undefined, up|down|undefined}].
menu_flat(Menu, Context) ->
    menu_flat(Menu, 999, Context).

-spec menu_flat(list() | undefined | <<>>, integer(), #context{}) ->
    [{integer()|undefined, LevelIndex :: list(integer()) | undefined, up|down|undefined}].
...
-spec menu_subtree(Menu :: list(), PageId :: term(), AddSiblings :: boolean(), #context{}) ->
    list() | undefined.
...
menu_subtree_1([{BelowId, Menu} | _Rest], BelowId, false, _CurrMenu, _Context) ->
    Menu;
menu_subtree_1([{Id, Menu} | Rest], BelowId, AddSiblings, CurrMenu, Context)
